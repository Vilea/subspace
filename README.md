...
| `SUBSPACE_TUNNEL_DNS_SERVERS`   | `false`             | When set to "true", ensure DNS servers end up in allowed IPs. This is configurable in case the DNS servers are explicit and public and split-tunneling is desired. This is mostly useful for IPv6, because in most cases (?) the whole IPv4 network is added to AllowedIPs already. |
